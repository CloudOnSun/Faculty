...
class MyLogisticRegression:
        ...
        for epoch in range(noEpochs):
            ...
            for j in range(0, len(x[0])):
                ...
                for i in range(len(x)):
                    s += crtErrors[i] * x[i][j]
                coefDeriv.append(s / len(x))

            for j in range(0, len(x[0])):  # update the coefficients
                self.coef_[j] = self.coef_[j] - learningRate * coefDeriv[j] * sigmoid(self.eval(x[j], self.coef_)) * (1 -
                     sigmoid(self.eval(x[j], self.coef_)))

            self.coef_[len(x[0])] = self.coef_[len(x[0])] - learningRate * (sum(crtErrors) / len(crtErrors))
    ...
    def predictFloat(self, inTest):
        coefficients = [self.intercept_] + [c for c in self.coef_]
        comutedLabels = [self.eval(sample, coefficients) for sample in inTest]
        return comutedLabels