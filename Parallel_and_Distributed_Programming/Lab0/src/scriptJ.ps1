...
for ($i = 0; $i -lt $param3; $i++){
    ...
    $a = java $args[0] $args[1] $args[3] $args[4] $args[5] $args[6] $args[7] # rulare class java
    Write-Host $a
    $suma += $a
...
if (!(Test-Path outJ.csv)){
    ...
    #Scrie date in csv
    Set-Content outJ.csv 'Tip Program,Nr threads,Timp executie,No of Runs,FisierIntrare'
}

# Append
Add-Content outJ.csv "$($args[4]),$($args[1]),$($media),$($args[2]),$($args[3])"