...
namespace Lab7
{
    public class Cell
    {
        ...
        public Cell(StringBuilder data)
        {
            this.data = data;
        }
    ...
    public class CellNotResolved : Cell
    {
        ...
        public CellNotResolved(StringBuilder data, int operation, int leftIndex1, int leftIndex2, int rightIndex1, int rightIndex2) : base(data)
        { 
    ...
    public class CellHandler
    {
        ...
        public static Tuple<string, int, int, int, int> indexCalc(string[] cell)
        {
            var opL = cell[0];
            var opR = cell[1];
            int l1 = 0;
            ...
            //if we didn't have an idex for every row and coloumn but we reached end of stream it means we have only capital letters or onlt numbers
            if (i != -1 || j != -1 || l1 == 0 || l2 == 0 || r1 == 0 || r2 == 0)
            {
                return new Tuple<string, int, int, int, int>("#FORMULA", -1, -1, -1, -1);
            }
            else
                return new Tuple<string, int, int, int, int>("", l1, l2, r1, r2);
    ...
    public class SheetCalc
    {
        ...
        {
            cell.setData(new StringBuilder("#CYCLE"));
            cycle.Remove(cell);
        ...
        private void calcCell (List<List<Cell>> sheet, CellNotResolved cell)
        {
            ...
            if (cell.notResolved())
            {
                ...
                if (dataL == -1)
                {
                    StringBuilder textL = lCell.data;
                    //some kind of an error
                    if (textL[0] == '#')
                    {
                        cell.setData(new StringBuilder("#ERROR"));
                        cycle.Clear();
                        return;
                    }
                    if (textL.Equals("[]"))
                ...
                if (op == 3 && dataR == 0)
                {
                    cell.setData(new StringBuilder("#DIV0"));
                    cycle.Clear();
                    return;
                }

                var res = CellHandler.calcCell(dataL, dataR, op);
                cell.setData(new StringBuilder(res.ToString()));
                cycle.Clear();
        ...
        private void readSheet()
        {
            ...
            while (line != null)
            {
                ...
                foreach (var cell in cells) // process each cell from the line
                {
                    if (cell.Equals("[]"))
                    {
                        cellList.Add(new Cell(new StringBuilder("[]")));
                        continue;
                    }
                    if (cell[0] == '=') // a formula
                    {
                        ...
                        foreach (var oper in operators) // try to find if it contains an operator
                        {
                            ...
                            {
                                var operands = cell.Split(new char[] { oper, '=' }, StringSplitOptions.RemoveEmptyEntries); // find the operands
                                if (operands.Length != 2)
                                {
                                    cellList.Add(new Cell(new StringBuilder("#FORMULA")));
                                }
                                else
                                {
                                    ...
                                    {
                                        cellList.Add(new Cell(new StringBuilder(tuple.Item1))); // adds data = "#FORMULA"
                                    }
                                    else
                                    {
                                        cellList.Add(new CellNotResolved(new StringBuilder(cell), op, tuple.Item2-1, tuple.Item3-1, tuple.Item4-1, tuple.Item5-1));
                                    }
                                }
                                break;
                            }
                        ...
                        {
                            cellList.Add(new Cell(new StringBuilder("#MISSOP")));
                        }
                    }
                    else
                    {
                        ...
                        {
                            cellList.Add(new Cell(new StringBuilder(Convert.ToInt32(cell).ToString())));
                        }
                        catch (FormatException) // not a number
                        {
                            cellList.Add(new Cell(new StringBuilder("#INVVAL")));
...
// TODO: #ERROR poate fi folosit peste tot
// TODO: dictionar pt sheeturi
// TODO: without set for cycle